...
locals {
  ...
  subnets = {
    "management-subnet"       = { address_prefixes = [cidrsubnet(local.cidr_map[local.environment], 8, 1)], nsg_name = "nsg-mgmt-prod" }
    "application-subnet"      = { address_prefixes = [cidrsubnet(local.cidr_map[local.environment], 8, 2)], nsg_name = "nsg-app-prod" }
    "data-subnet"             = { address_prefixes = [cidrsubnet(local.cidr_map[local.environment], 8, 3)], nsg_name = "nsg-data-prod" }
